...
def validate_txt(test_dataset, testfilename, model, batch_size=64):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start, len(test_dataset))):
            ...
            # aug_name = im_name.split('.')[0] + '-' + str(aug_id) + '.jpg'

            '''
            #debug stuff. Doesn't work because of normalization
            save_name = im_name.split('/')[-1].split('.')[0] + '-' + str(aug_id) + '.jpg'
            save_path = '/scratch/ssolit/StreamingPerception/sst_augmentation/img_gen/gen-s2-1/' + aug_name
            print(save_path)
            #cv2.imwrite(save_path, np.transpose(image.numpy(), axes=(1,2,0)) )
            '''
